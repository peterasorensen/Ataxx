...
class Move {
    ...
    boolean isExtend() {
        return (abs(col1() - col0()) == 1 || abs(row1() - row0()) == 1)
                && (abs(col1() - col0()) <= 1 && abs(row1() - row0()) <= 1);
    }

    /** Return true if thIs is a jump (move to adjacent square). */
    boolean isJump() {
        return (abs(col1() - col0()) == 2 || abs(row1() - row0()) == 2)
                && (abs(col1() - col0()) <= 2 && abs(row1() - row0()) <= 2);
    }
